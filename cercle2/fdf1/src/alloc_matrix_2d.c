...
t_bool	alloc_matrix_2d(t_all_matrix *all_matrix)
{
	...
	if (!all_matrix -> display_matrix -> display_pixel)
		return (free(all_matrix -> display_matrix), all_matrix -> display_matrix = NULL, FALSE);
	i = -1;
	while (++i < height)
	{
		all_matrix ->display_matrix->display_pixel[i] =  malloc(sizeof(t_point_3d) *all_matrix ->matrix_3_d->width);
		if (!all_matrix ->display_matrix->display_pixel[i])
			return (free_display_matrix(all_matrix ->display_matrix),all_matrix ->display_matrix = NULL, FALSE);
	}
	return (TRUE);
}