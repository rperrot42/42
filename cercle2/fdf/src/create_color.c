...
int	create_color(unsigned char t, unsigned char r, unsigned char g, \
unsigned char b)
{
...
t_info_segment *info_segment)
{
	...
	while (++i < 4)
	{
		...
		{
			info_segment->d_color[i] = ((b->color) >> (8 * i) & 255) - \
	((a->color) >> (8 * i) & 255);
		...
		{
			info_segment->avanc_color[i] = -1;
			info_segment->d_color[i] = ((a->color) >> (8 * i) & 255) - \
	((b->color) >> (8 * i) & 255);
		}
	}
...
void	create_color_line(t_point_3d *a, t_info_segment *info_segment)
{
	...
	while (++i < 4)
	{
		if (info_segment->d_color[i])
		{
			info_segment->eps_color[i] += info_segment->d_color[i];
			if (info_segment->eps_color[i] * 2 > info_segment->dx)
			{
				a->color += (info_segment->eps_color[i] / info_segment->dx * \
info_segment->avanc_color[i]) << (8 * i);
				info_segment->eps_color[i] -= info_segment->eps_color[i] \
	/ info_segment->dx * info_segment->dx;
				a->color += ((info_segment->eps_color[i] * 2 > \
info_segment->dx) * info_segment->avanc_color[i]
						<< (8 * i));
				info_segment->eps_color[i] -= \
	(info_segment->eps_color[i] * 2 > info_segment->dx) * info_segment->dx;
			}
		}
	}
